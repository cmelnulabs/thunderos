...
.. list-table::
   ...
   * - ``dma_alloc()``
     - O(n + m)
     - n = pages to allocate, m = existing regions
   * - ``dma_free()``
     - O(m)
     - m = existing regions (linked list traversal)
   * - ``dma_phys_addr()``
     - O(1)
     - Direct field access
   * - ``dma_virt_addr()``
...
* PMM page allocation: O(n) where n = number of pages
* Memory zeroing (if DMA_ZERO): O(n × PAGE_SIZE) = O(total_bytes)
* Linked list append: O(m) where m = number of existing allocations
* Total: O(n + m)
...
.. code-block:: text
   ...
   sizeof(dma_region_t) = 32 bytes
   
   Components:
   - virt_addr: 8 bytes (pointer)
   - phys_addr: 8 bytes (uintptr_t)
   - size:      8 bytes (size_t)
